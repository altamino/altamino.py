...
if TYPE_CHECKING:
    from altamino.utils.requester import AsyncHTTPResponse, HTTPResponse, HTTPRequest
...
async def checkAsyncException(response: AsyncHTTPResponse):
	try:
		...
		apiCode = data.get("api:statuscode")
		message = data.get("api:message")
	except JSONDecodeError:
		raise UnknownError(await response.text(), response)
	if apiCode in exceptions_list: raise exceptions_list[apiCode](message, response)
