...
class GlobalChatsModule(AsyncBaseClass):
	...
	async def join_chat(self, chatId: str) -> resp.BaseObject:
		...
		**Parameters**
		- chatId : ID of the Chat.
		"""
		return resp.BaseObject(await (await self.req.make_async_request("POST", f"/g/s/chat/thread/{chatId}/member/{self.userId}", headers={"Content-Type": "application/x-www-form-urlencoded"})).json())
	...
	async def leave_chat(self, chatId: str) -> resp.BaseObject:
		...
		**Parameters**
		- chatId : ID of the Chat.
		"""
		return resp.BaseObject(await (await self.req.make_async_request("DELETE", f"/g/s/chat/thread/{chatId}/member/{self.userId}", headers={"Content-Type": "application/x-www-form-urlencoded"})).json())
